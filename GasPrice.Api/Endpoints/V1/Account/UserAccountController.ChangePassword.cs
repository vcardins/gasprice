...
namespace GasPrice.Api.Endpoints.V1.Account
{
    public partial class UserAccountController 
    {
        ...
        [ModelStateValidator]
        public IHttpActionResult PatchChangePassword(ChangePasswordInputModel model)
        {
            _userService.ChangePassword(User.GetUserID(), model.OldPassword, model.NewPassword);
            return Ok(new { Message = UserAccountActionMessages.SuccessOnUpdatingPassword.GetDescription() });
        }
